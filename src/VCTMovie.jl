...
function makeMovie(simulation_id::Int)
    path_to_output_folder = "$(data_dir)/outputs/simulations/$(simulation_id)/output"
    ...
    jpgs = readdir("$(data_dir)/outputs/simulations/$(simulation_id)/output", sort=false)
    filter!(f -> endswith(f, ".jpg"), jpgs)
    for jpg in jpgs
        rm("$(data_dir)/outputs/simulations/$(simulation_id)/output/$(jpg)")
    end
    return movie_generated
end
