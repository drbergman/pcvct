...
function moatSensitivity(method::MOAT, monad_min_length::Int, folder_names::AbstractSamplingFolders, avs::Vector{<:AbstractVariation}; reference_config_variation_id::Int=0, reference_rulesets_variation_id::Int=0, reference_ic_cell_variation_id::Int=folder_names.ic_cell_folder=="" ? -1 : 0, ignore_indices::Vector{Int}=Int[], force_recompile::Bool=true, prune_options::PruneOptions=PruneOptions())
    if !isempty(ignore_indices)
        error("MOAT does not support ignoring indices...yet? Only Sobolʼ does for now.")
    end
    config_variation_ids, rulesets_variation_ids, ic_cell_variation_ids = addVariations(method.lhs_variation, avs, folder_names.config_folder; 
        rulesets_collection_folder=folder_names.rulesets_collection_folder, ic_cell_folder=folder_names.ic_cell_folder,
    ...
    perturbed_config_variation_ids = repeat(config_variation_ids, 1, length(avs))
    perturbed_rulesets_variation_ids = repeat(rulesets_variation_ids, 1, length(avs))
    perturbed_ic_cell_variation_ids = repeat(ic_cell_variation_ids, 1, length(avs))
    for (base_point_ind, (config_variation_id, rulesets_variation_id, ic_cell_variation_id)) in enumerate(zip(config_variation_ids, rulesets_variation_ids, ic_cell_variation_ids)) # for each base point in the LHS
        for (par_ind, ev) in enumerate(avs) # perturb each parameter one time
            variation_location = variationLocation(ev)
            if variation_location == :config
            ...
            elseif variation_location == :rulesets
            ...
            elseif variation_location == :ic_cell
                ...
                error("Unknown variation location: $variation_location")
            end
        end
    end
    ...
    header_line = ["base", [columnName(av) for av in avs]...]
...
function perturbICCellVariation(ev::ElementaryVariation, ic_cell_variation_id::Int, folder::String)
    base_value = icCellValue(columnName(ev), ic_cell_variation_id, folder)
...
function icCellValue(ev::ElementaryVariation, ic_cell_variation_id::Int, folder::String)
    column_name = columnName(ev)
    return icCellValue(column_name, ic_cell_variation_id, folder)
end

function icCellValue(column_name::String, ic_cell_variation_id::Int, folder::String)
...
function getBaseValue(ev::ElementaryVariation, variation_id::Int, folder_names::AbstractSamplingFolders)
    ...
    elseif variation_location == :ic_cell
        return icCellValue(ev, variation_id, folder_names.ic_cell_folder)
    else
        error("Unknown variation location: $variation_location")
    end
end
...
function sobolSensitivity(method::Sobolʼ, monad_min_length::Int, folder_names::AbstractSamplingFolders,
                          avs::Vector{<:AbstractVariation}; reference_config_variation_id::Int=0,
                          reference_rulesets_variation_id::Int=0,
                          reference_ic_cell_variation_id::Int=folder_names.ic_cell_folder=="" ? -1 : 0,
                          ignore_indices::Vector{Int}=Int[], force_recompile::Bool=true,
    ...
    config_variation_ids, rulesets_variation_ids, ic_cell_variation_ids, cdfs, parsed_variations =
        _addVariations(method.sobol_variation, avs, config_id, rulesets_collection_id, ic_cell_id, reference_variation_ids)
    ...
    header_line = ["A", "B", [columnName(av) for av in avs[focus_indices]]...]
    monad_ids_df = DataFrame(monad_ids, header_line)
...
function rbdSensitivity(method::RBD, monad_min_length::Int, folder_names::AbstractSamplingFolders, avs::Vector{<:AbstractVariation}; reference_config_variation_id::Int=0, reference_rulesets_variation_id::Int=0, reference_ic_cell_variation_id::Int=folder_names.ic_cell_folder=="" ? -1 : 0, ignore_indices::Vector{Int}=Int[], force_recompile::Bool=true, prune_options::PruneOptions=PruneOptions())
    if !isempty(ignore_indices)
        error("RBD does not support ignoring indices...yet? Only Sobolʼ does for now.")
    end
    config_variation_ids, rulesets_variation_ids, ic_cell_variation_ids, config_variations_matrix, rulesets_variations_matrix, ic_cell_variations_matrix = 
        addVariations(method.rbd_variation, avs, folder_names.config_folder; rulesets_collection_folder=folder_names.rulesets_collection_folder,
        ic_cell_folder=folder_names.ic_cell_folder, reference_config_variation_id=reference_config_variation_id,
        reference_rulesets_variation_id=reference_rulesets_variation_id, reference_ic_cell_variation_id=reference_ic_cell_variation_id)
    ...
    monads = monad_dict |> values |> collect
    header_line = [columnName(av) for av in avs]
    monad_ids_df = DataFrame(monad_ids, header_line)
    sampling = Sampling(monad_min_length, monads)
    n_success = run(sampling; force_recompile=force_recompile, prune_options=prune_options)
