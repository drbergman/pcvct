...
struct Simulation <: AbstractMonad
    ...
    function Simulation(id::Int, folder_ids::AbstractSamplingIDs, folder_names::AbstractSamplingFolders, variation_ids::VariationIDs)
        ...
        if variation_ids.ic_cell == -1
            @assert folder_names.ic_cell_folder == "" "ic_cell_folder must not be provided if ic_cell variation id is -1 (indicating that the initial cells are not in use)"
        else
            @assert folder_names.ic_cell_folder != "" "ic_cell_folder must be provided if ic_cell_variation_id is not -1 (> -1 indicating that the cells are in use)"
            @assert variation_ids.ic_cell == 0 || isfile(joinpath(data_dir, "inputs", "ics", "cells", folder_names.ic_cell_folder, "cells.xml")) "cells.xml must be provided if ic_cell_variation_id is >=1 (indicating that the cell ic parameters are being varied)"
        end
...
function Simulation(folder_ids::AbstractSamplingIDs, folder_names::AbstractSamplingFolders, variation_ids::VariationIDs)
    ...
    $(join(fieldnames(pcvct.VariationIDs) .* "_variation_id", ",")),\
    status_code_id) \
    VALUES(\
...
function Sampling(monad_min_length::Int, monad_ids::AbstractVector{<:Integer}, folder_ids::AbstractSamplingIDs, folder_names::AbstractSamplingFolders, variation_ids::Vector{VariationIDs})
    id = -1
...
function createMonadIDs(monad_min_length::Int, folder_ids::AbstractSamplingIDs, folder_names::AbstractSamplingFolders, variation_ids::AbstractArray{VariationIDs}; use_previous_simulations::Bool=true)
